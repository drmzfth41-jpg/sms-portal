...
exports.handler = async (event) => {
  ...
  const {
    numbers, sender = DEFAULT_SENDER, title = DEFAULT_SENDER,
    encoding = 1, validity = 1440, commercial = false,
    sendingDate = null, pushUrl = null,
  } = JSON.parse(event.body || '{}');

  // numbers: [{ nr: "905...", msg: "..." }, ...]
  if (!numbers?.length) return respond(400, { error: 'numbers zorunludur.' });

  const cleaned = numbers.map(item => ({
    nr:  String(item.nr).replace(/\D/g, ''),
    msg: item.msg || '',
  }));

  const raw = await izbilFetch('POST', '/sms/create', {
    type: 1, sendingType: 2,
